...
1. **高画質化・鮮明化**  
   ...
   **※ 引用について** 本ドキュメントにおける動作例の画面では、学生に人気のある著名な書籍である[「民法 I 第 4 版: 総則・物権総論」](https://www.amazon.co.jp/dp/4130323512/) (著者: 内田貴先生、ISBN: 4130323512、2008 年 4 月出版、電子書籍未出版) の P.226 ～ P.227 のページ部分のごく一部の切り抜き部分を引用し、処理前と処理後の結果を示します。この 18 年前の名著を処理のサンプルとして用いる理由は、本書にはデザイン上いろいろな特徴があり、本ソフトウェアを作る際に乗り越える必要がある必要な追加的処理が高密度に出現することから、本ソフトウェアの開発においてリファレンスとして有用であったことによります。たとえば、ページ番号部分に特有の飾り文字 (後述) が付いており、ページ番号自動検出時のアルゴリズムを実装する際に追加的処理を必要とする特殊なデザインになっています (言い換えれば、この書籍のページ番号部分の読み取りに成功すれば、たいていの書籍のページ番号部分の読み取りに成功します)。
...
6. **見開き表示用に左側ページと右側ページとを自動判別**  
   ...
   に表示されるべきかのフラグを、前述のページ番号自動検出の結果に基づき、PDF 内に埋め込むようにしました。これにより、これまでおおむね 1/2 の確率で PDF ビューアの設定を変更したりあるいは空白ページを P.1 の前に差し込んで調整したりするような作業が不要になります。
